...
bool HackRFDevice::set_rx_gain( double rx_gain )
{
    ...
    std::cout << "HackRFDevice: Rx gain parts: LNA: " << lna_part << "  VGA: " << vga_part << std::endl;
    
    set_lna_gain( lna_part );
    set_rxvga_gain( vga_part );
...
bool HackRFDevice::set_rxvga_gain( uint32_t rxvga_gain )
{
    if ( m_is_initialized )
    {
        ...
        {
            std::cout << "HackRFDevice: Setting Rx VGA gain " << rxvga_gain << std::endl;
