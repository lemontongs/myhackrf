...
bool SignalSaver::start_Rx( hackrf_sample_block_cb_fn callback, void* args = NULL )
{
...
bool SignalSaver::start_Tx( hackrf_sample_block_cb_fn callback, void* args = NULL )
{
    if ( m_is_initialized && m_device_mode == STANDBY_MODE )
    {
...
bool SignalSaver::tune( uint64_t fc_hz )
{
    if ( m_is_initialized )
    {
...
bool SignalSaver::set_sample_rate( double fs_hz )
{
    if ( m_is_initialized )
    {
        switch ( int( fs_hz ) )
...
bool SignalSaver::force_sample_rate( double fs_hz )
{
    if ( m_is_initialized )
    {
...
bool SignalSaver::set_lna_gain( uint32_t lna_gain )
{
    if ( m_is_initialized )
    {
        switch ( lna_gain )
...
bool SignalSaver::set_rxvga_gain( uint32_t rxvga_gain )
{
    if ( m_is_initialized )
    {
...
bool SignalSaver::set_txvga_gain( uint32_t txvga_gain )
{
    if ( m_is_initialized )
    {
...
bool SignalSaver::set_amp_enable( uint8_t amp_enable )
{
    if ( m_is_initialized )
    {
...
bool SignalSaver::set_antenna_enable( uint8_t antenna_enable )
{
    if ( m_is_initialized )
    {
...
bool SignalSaver::check_error(int error)
{
